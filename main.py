...
class TradeBlockWindow(tk.Toplevel):
    """Simple dialog for selecting a player for a line slot."""
    
    def __init__(self, parent, available_players, line_type, line_index, slot_index):
        super().__init__(parent)
        self.parent = parent
        ...
        self.configure(bg=parent.parent.BG_COLOR)
    ...
    def create_interface(self):
        ...
        if self.line_type in position_names:
            if self.line_type == 'goalie':
                pos_text = "G"
            else:
                pos_text = position_names[self.line_type][self.slot_index]
        else:
            pos_text = "Player"
        
        ttk.Label(header_frame, text=f"Select {pos_text} for {self.get_line_title()}",
                 style='SubTitle.TLabel', font=(self.parent.parent.FONT_FAMILY, 12, 'bold')).pack()
        
        # Player list
        list_frame = ttk.Frame(self, style='Panel.TFrame', padding=15)
        list_frame.pack(fill=tk.BOTH, expand=True)
        
        # Create listbox with scrollbar
        listbox_frame = ttk.Frame(list_frame, style='Panel.TFrame')
        listbox_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 15))
        
        self.player_listbox = tk.Listbox(listbox_frame, height=15, 
                                        bg=self.parent.parent.CONTENT_BG,
                                        fg=self.parent.parent.TEXT_COLOR,
                                        font=(self.parent.parent.FONT_FAMILY, 10),
                                        selectbackground=self.parent.parent.ACCENT_COLOR,
                                        relief='flat')
        ...
        # Populate listbox
        for player in sorted(self.available_players, key=lambda p: p.overall_rating(), reverse=True):
            display_text = f"{player.full_name} (OVR: {player.overall_rating()})"
            self.player_listbox.insert(tk.END, display_text)
        
        # Add "Remove Player" option
        self.player_listbox.insert(0, "--- Remove Player from Slot ---")
        
        self.player_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        # Double-click to select
        self.player_listbox.bind('<Double-Button-1>', self.on_select)
        ...
        ttk.Button(button_frame, text="Select", command=self.on_select,
                  style='Accent.TButton').pack(side=tk.RIGHT)
    
    def get_line_title(self):
        """Get the title for the current line/pair."""
        if self.line_type == 'forward':
            return f"Line {self.line_index + 1}"
        elif self.line_type == 'defense':
            return f"Pair {self.line_index + 1}"
        else:
            return "Starter" if self.line_index == 0 else "Backup"
    ...
    def on_select(self, event=None):
        ...
        # Handle removal (first option)
        if index == 0:
            selected_player = None
        else:
            # Get the selected player (adjust for the "remove" option)
            selected_player = self.available_players[index - 1]
        
        # Update the lineup
        if self.line_type == 'forward':
            if 'Forwards' not in self.parent.lineup:
                self.parent.lineup['Forwards'] = [[None]*3 for _ in range(4)]
            self.parent.lineup['Forwards'][self.line_index][self.slot_index] = selected_player
        elif self.line_type == 'defense':
            if 'Defense' not in self.parent.lineup:
                self.parent.lineup['Defense'] = [[None]*2 for _ in range(3)]
            self.parent.lineup['Defense'][self.line_index][self.slot_index] = selected_player
        elif self.line_type == 'goalie':
            if 'Goalies' not in self.parent.lineup:
                self.parent.lineup['Goalies'] = [None, None]
            self.parent.lineup['Goalies'][self.line_index] = selected_player
        
        self.destroy()
    
    def update_all_displays(self):
        """Update all line displays with current lineup."""
        # Update forward cards
        for i, card in enumerate(self.forward_cards):
            self.update_line_card(card, 'forward', i, 3)
        
        # Update defense cards
        for i, card in enumerate(self.defense_cards):
            self.update_line_card(card, 'defense', i, 2)
        
        # Update goalie cards
        for i, card in enumerate(self.goalie_cards):
            self.update_line_card(card, 'goalie', i, 1)
        
        # Update available players
        self.update_available_players()
    
    def update_line_card(self, card, line_type, line_index, num_slots):
        """Update a specific line card with current data."""
        slot_labels = getattr(card, 'slot_labels', [])
        rating_label = getattr(card, 'rating_label', None)
        
        # Update player slots
        for i in range(min(len(slot_labels), num_slots)):
            player = self.get_player_for_slot(line_type, line_index, i)
            
            if player:
                player_text = f"{player.full_name}\n{player.primary_position.value} • OVR {player.overall_rating()}"
                text_color = self.parent.TEXT_COLOR
                bg_color = '#1F1F1F'
            else:
                if line_type == 'forward':
                    positions = ['LW', 'C', 'RW']
                    player_text = f"Empty Slot\n{positions[i]}"
                elif line_type == 'defense':
                    positions = ['LD', 'RD']
                    player_text = f"Empty Slot\n{positions[i]}"
                else:  # goalie
                    player_text = "Empty Slot\nG"
                text_color = '#666666'
                bg_color = '#1A1A1A'
            
            # Update the label (it's a tk.Label now, not ttk.Label)
            slot_labels[i].config(text=player_text, fg=text_color, bg=bg_color)
        
        # Update line rating
        if rating_label:
            line_rating = self.calculate_line_rating(line_type, line_index)
            rating_label.config(text=f"OVR: {line_rating}")
    
    def update_available_players(self):
        """Update the available players list with organized tabs."""
        # Get all assigned players
        assigned = set()
        for line in self.lineup.get('Forwards', []):
            for player in line:
                if player:
                    assigned.add(player)
        for pair in self.lineup.get('Defense', []):
            for player in pair:
                if player:
                    assigned.add(player)
        for goalie in self.lineup.get('Goalies', []):
            if goalie:
                assigned.add(goalie)
        
        # Get unassigned players
        unassigned = [p for p in self.roster if p not in assigned]
        total_available = len(unassigned)
        
        # Update count label
        self.available_count_label.config(text=f"{total_available} players available")
        
        # Update each position tab
        for group_name, group_data in self.position_frames.items():
            ...
            # Get players for this position group
            group_players = [p for p in unassigned if p.primary_position.name in positions]
            group_players.sort(key=lambda p: p.overall_rating(), reverse=True)
            
            # Populate listbox
            for player in group_players:
                display_text = f"{player.full_name} ({player.overall_rating()})"
                listbox.insert(tk.END, display_text)
            
            # Store player references for quick access
            group_data['players'] = group_players
    
    def quick_add_player(self, listbox):
        """Quick-add a player to an appropriate empty slot."""
        selection = listbox.curselection()
        if not selection:
            return
        ...
        # Find which position group this listbox belongs to
        selected_player = None
        for group_name, group_data in self.position_frames.items():
            if group_data['listbox'] == listbox:
                if 'players' in group_data and index < len(group_data['players']):
                    selected_player = group_data['players'][index]
                break
        ...
        position = selected_player.primary_position.name
        
        # Try to find empty slot based on position
        if position in ['LEFT_WING', 'CENTER', 'RIGHT_WING']:
            # Find empty forward slot
            slot_map = {'LEFT_WING': 0, 'CENTER': 1, 'RIGHT_WING': 2}
            preferred_slot = slot_map.get(position, 0)
            
            for line_idx in range(4):
                # Try preferred position first
                if self.get_player_for_slot('forward', line_idx, preferred_slot) is None:
                    self.assign_player_to_slot(selected_player, 'forward', line_idx, preferred_slot)
                    return
                
                # Try any empty slot in the line
                for slot_idx in range(3):
                    if self.get_player_for_slot('forward', line_idx, slot_idx) is None:
                        self.assign_player_to_slot(selected_player, 'forward', line_idx, slot_idx)
                        return
        
        elif position in ['LEFT_DEFENSE', 'RIGHT_DEFENSE', 'DEFENSE']:
            # Find empty defense slot
            preferred_slot = 0 if position == 'LEFT_DEFENSE' else 1
            if position == 'DEFENSE':
                preferred_slot = 0  # Default to left side for general defensemen
            
            for pair_idx in range(3):
                # Try preferred side first
                if self.get_player_for_slot('defense', pair_idx, preferred_slot) is None:
                    self.assign_player_to_slot(selected_player, 'defense', pair_idx, preferred_slot)
                    return
                
                # Try other side
                other_slot = 1 - preferred_slot
                if self.get_player_for_slot('defense', pair_idx, other_slot) is None:
                    self.assign_player_to_slot(selected_player, 'defense', pair_idx, other_slot)
                    return
        
        elif position == 'GOALIE':
            # Find empty goalie slot
            for goalie_idx in range(2):
                if self.get_player_for_slot('goalie', goalie_idx, 0) is None:
                    self.assign_player_to_slot(selected_player, 'goalie', goalie_idx, 0)
                    return
        
        # If no slot found, show message
        messagebox.showinfo("No Empty Slots", f"No empty slots available for {selected_player.full_name}")
    
    def assign_player_to_slot(self, player, line_type, line_index, slot_index):
        """Assign a player to a specific slot."""
        if line_type == 'forward':
            if 'Forwards' not in self.lineup:
                self.lineup['Forwards'] = [[None]*3 for _ in range(4)]
            self.lineup['Forwards'][line_index][slot_index] = player
        elif line_type == 'defense':
            if 'Defense' not in self.lineup:
                self.lineup['Defense'] = [[None]*2 for _ in range(3)]
            self.lineup['Defense'][line_index][slot_index] = player
        elif line_type == 'goalie':
            if 'Goalies' not in self.lineup:
                self.lineup['Goalies'] = [None, None]
            self.lineup['Goalies'][line_index] = player
        
        self.update_all_displays()
    
    def clear_all_lines(self):
        """Clear all player assignments."""
        if messagebox.askyesno("Clear Lines", "Are you sure you want to clear all lines?"):
            self.lineup = {
                'Forwards': [[None]*3 for _ in range(4)],
                'Defense': [[None]*2 for _ in range(3)],
                'Goalies': [None, None]
            }
            self.update_all_displays()
    
    def save_and_close(self):
        """Save lines and close the window."""
        self.save_lines()
        self.destroy()
    ...
    def save_lines(self):
        ...
        messagebox.showinfo("Lines Saved", "Your line configuration has been saved successfully!")

    def set_best_lines(self):
        """Set all lines to the best possible configuration."""
        if messagebox.askyesno("Auto-Set Lines", "This will automatically set your best possible lines. Continue?"):
            best = best_lines(self.parent.user_team)
            self.lineup = best
            self.parent.user_team.lineup = best
            self.update_all_displays()
            messagebox.showinfo("Lines Updated", "Your team's best lines have been set!")

    def reset_lines(self):
        """Reset lines to best lines configuration."""
        if messagebox.askyesno("Reset Lines", "This will reset all lines to the best possible configuration. Continue?"):
            best = best_lines(self.parent.user_team)
            self.lineup = best
            self.parent.user_team.lineup = best
            self.update_all_displays()
            messagebox.showinfo("Lines Reset", "Lines have been reset to optimal configuration!")
...
class TradeBlockWindow(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        tk.Entry(filter_panel, textvariable=self.filter_vars['min_ovr'], width=4, bg=self.CONTENT_BG, fg=self.TEXT_COLOR, insertbackground=self.TEXT_COLOR).pack(side="left", padx=2)
        ttk.Label(filter_panel, text="Max Age:").pack(side="left")
        tk.Entry(filter_panel, textvariable=self.filter_vars['max_age'], width=4, bg=self.CONTENT_BG, fg=self.TEXT_COLOR, insertbackground=self.TEXT_COLOR).pack(side="left", padx=2)
